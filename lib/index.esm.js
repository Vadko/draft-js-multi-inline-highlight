...
var Fragmenter = /*#__PURE__*/function () {
  ...
  _proto.add = function add(nick, range) {
    if (!this.data[nick]) {
...
function LengthMatcher(fragmenter, length, style, contentBlock) {
...
function WordMatcher(fragmenter, items, style, contentBlock) {
  var text = contentBlock.getText();
...
function ExactWordMatcher(fragmenter, items, style, contentBlock) {
  var text = contentBlock.getText();
...
function SentenceMatcher(fragmenter, items, style, contentBlock) {
  var text = contentBlock.getText();
...
function MultiHighlightDecorator(config) {
  ...
  return new SimpleDecorator(function strategy(contentBlock, callback) {
    ...
    if (fragments.isMultiply()) {
      ...
      if (singleRanges) {
        for (var _iterator4 = _createForOfIteratorHelperLoose(singleRanges.range), _step4; !(_step4 = _iterator4()).done;) {
          ...
          callback(_range[0], _range[1], config.styles[singleRanges.style]);
        }
      }
    }
...
export { ExactWordMatcher, LengthMatcher, MultiHighlightDecorator, SentenceMatcher, WordMatcher };
